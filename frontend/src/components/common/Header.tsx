...
export const Header: React.FC = () => {
  ...
  const roleInfo = roleLabelMap[user?.role || 'DISPATCHER'] || { label: user?.role || 'User', badge: 'bg-slate-100 text-slate-700 border-slate-200' };

  return (
    <header className="h-14 border-b border-slate-200 bg-white px-6 flex items-center justify-between sticky top-0 z-30">
      {/* Left Operations Status */}
      <div className="flex items-center space-x-5">
        ...
        <div className="hidden md:flex items-center space-x-2 px-2.5 py-1 rounded bg-slate-50 border border-slate-200 text-xs text-slate-600">
          <Clock className="w-3.5 h-3.5 text-slate-400" />
          <span>Hub time (IST):</span>
